...
export function filterData(
  ...
  const filtered = data.filter((record) => {
    ...
    // Only skip the geography filter if NO geographies are selected
    // If a specific geography (like "U.S.") is selected, we should filter by it
    let skipGeoFilterForRegional = isRegionalSegmentType && filters.geographies.length === 0

    // For "By Region", expand parent geographies to include their child regions
    // Since parent_geography is often null, we need a manual mapping
    let expandedGeographies = [...filters.geographies]
    if (isRegionalSegmentType && filters.geographies.length > 0) {
      const geoExpansionMap: Record<string, string[]> = {
        'U.S.': ['U.S.', 'Northeast', 'Southeast', 'West', 'Midwest', 'Southwest'],
        'North America': ['North America', 'U.S.', 'Canada', 'Northeast', 'Southeast', 'West', 'Midwest', 'Southwest'],
        'Europe': ['Europe', 'U.K.', 'Germany', 'Italy', 'France', 'Spain', 'Russia', 'Rest of Europe'],
        'Asia Pacific': ['Asia Pacific', 'China', 'India', 'Japan', 'South Korea', 'ASEAN', 'Australia', 'Rest of Asia Pacific'],
      }

      filters.geographies.forEach(geo => {
        if (geoExpansionMap[geo]) {
          expandedGeographies.push(...geoExpansionMap[geo])
        }
      })
      expandedGeographies = [...new Set(expandedGeographies)] // Remove duplicates

      console.log('🔍 GEO EXPANSION: Expanded geographies for By Region:', {
        original: filters.geographies,
        expanded: expandedGeographies
      })
    }

    let geoMatch = filters.geographies.length === 0 ||
      expandedGeographies.includes(record.geography) ||
      skipGeoFilterForRegional // Only skip geography filter if no geography selected
    ...
    if (!geoMatch && record.parent_geography && filters.geographies.includes(record.parent_geography)) {
      geoMatch = true
      console.log('🔍 GEO FILTER: Parent geography match:', {
        recordGeo: record.geography,
        recordSegment: record.segment,
        parentGeo: record.parent_geography,
        selectedGeos: filters.geographies
      })
    }
    ...
    if (!geoMatch) {
      console.log('🔍 FILTER: Geography mismatch, excluding record:', {
        geography: record.geography,
        parent_geography: record.parent_geography,
    ...
    if (filters.advancedSegments && filters.advancedSegments.length > 0) {
      ...
      if (effectiveAggregationLevel === 1) {
        ...
        segmentMatch = filters.advancedSegments.some(seg => {
          console.log('🔍 SEGMENT FILTER: Checking record:', {
            recordSegmentType: record.segment_type,
            recordSegment: record.segment,
            recordGeography: record.geography,
            selectedSegmentType: seg.type,
            selectedSegment: seg.segment,
            isRegionSegmentType
          })

          if (seg.type !== record.segment_type) {
            console.log('🔍 SEGMENT FILTER: Type mismatch, skipping')
            return false
          }
          ...
          // Special handling for "By Region" - check if selected segment is a parent geography
          // For "By Region", check if the selected segment (e.g., "U.S.") matches the record's geography
          if (isRegionSegmentType) {
            // First check if selection matches geography
            if (seg.segment === record.geography) {
              console.log('🔍 SEGMENT FILTER: By Region geography parent match:', {
                recordGeography: record.geography,
                recordSegment: record.segment,
                selectedSegment: seg.segment,
                hierarchy: record.segment_hierarchy
              })
              return true
            }
          }
