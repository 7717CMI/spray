...
export function D3BubbleChartIndependent({ title, height = 500 }: BubbleChartProps) {
  ...
  // Clean up the hierarchy to ensure proper nesting (same as EnhancedFilterPanel)
  // This removes leaf nodes from appearing at the root level
  if (isOpportunityMode && Object.keys(hierarchy).length > 0) {
    const newHierarchy: Record<string, string[]> = {}

    // First, collect all children to identify leaf nodes
    const allChildren = new Set<string>()
    Object.entries(hierarchy).forEach(([_key, children]) => {
      if (Array.isArray(children)) {
        children.forEach(child => allChildren.add(child))
      }
    })

    // For B2B/B2C hierarchies, skip the business type root
    const businessTypeRoot = (hasB2BSegmentation && (activeFilters.businessType === 'B2B' || activeFilters.businessType === 'B2C'))
      ? activeFilters.businessType
      : null

    // Copy entries except:
    // 1. The business type root (B2B/B2C) if applicable
    // 2. Leaf nodes that are children of something else
    Object.entries(hierarchy).forEach(([key, value]) => {
      // Skip business type root if applicable
      if (businessTypeRoot && key === businessTypeRoot) {
        return
      }

      // Check if this is a self-referencing leaf node (e.g., "Northeast": ["Northeast"])
      const isSelfReferencing = Array.isArray(value) &&
        value.length === 1 &&
        value[0] === key

      // Only include if it has REAL children (not self-reference) OR if it's not a child of anything
      const hasRealChildren = Array.isArray(value) && value.length > 0 && !isSelfReferencing
      const isChildOfSomething = allChildren.has(key)

      if (hasRealChildren || !isChildOfSomething) {
        newHierarchy[key] = value
      }
    })
  ...
  const handleCascadeSelection = (path: string[]) => {
    console.log('🎯 handleCascadeSelection called:', {
      path,
      selectedSegmentType,
      isOpportunityMode
    })
    ...
    // Automatically update filters with the selected path to show data immediately
    if (path.length > 0) {
      const segmentToAdd = path[path.length - 1]
      const tempSegment: SelectedSegmentItem = {
        type: selectedSegmentType,
        segment: segmentToAdd,
        id: `${selectedSegmentType}::${segmentToAdd}`
      }

      console.log('🎯 Updating filters with segment:', {
        segmentToAdd,
        tempSegment
      })

      // Update filters immediately (without adding to selectedSegments permanently)
      // This allows seeing data in the chart before clicking "Add"
      updateActiveFilters({
        segments: [segmentToAdd],
        advancedSegments: [tempSegment],
      } as any)
    } else {
      console.log('🎯 Clearing cascade selection, restoring selectedSegments')
      // Clear filters when path is cleared
      updateActiveFilters({
        segments: selectedSegments.map(s => s.segment),
        advancedSegments: selectedSegments,
      } as any)
    }
  }
  ...
  const chartData = useMemo(() => {
    ...
    if (isOpportunityMode) {
      ...
      // When By Region is selected, automatically include all regions for selected geographies
      // This allows showing regions without requiring explicit segment selection
      const isRegionalSegmentType = activeFilters.segmentType === 'By Region' ||
                                     activeFilters.segmentType === 'By State' ||
                                     activeFilters.segmentType === 'By Country'

      let effectiveFilters = { ...activeFilters }

      if (isRegionalSegmentType && activeFilters.segments.length === 0 && activeFilters.geographies.length > 0) {
        // For By Region with no segments selected, don't filter by segments at all
        // Just show all segments that match the geography and segment type
        console.log('🎯 By Region mode: No segments selected, showing all regions for selected geographies')
        // Remove segment filtering by setting segments to empty and advancedSegments to empty
        // This will allow filterData to show all segments of this type
      }

      // Use filterData to apply all filters consistently with other charts
      let filteredRecords = filterData(dataset, effectiveFilters)
      ...
      if (activeFilters.segments && activeFilters.segments.length > 0) {
        ...
        if (isByRegionSegmentType) {
          ...
          const subRegions = ['U.S.', 'Canada', 'U.K.', 'Germany', 'France', 'Italy', 'Spain', 'Russia', 'China', 'India', 'Japan', 'South Korea', 'Australia']

          // Check if a specific region is selected (not Global, not multiple regions)
          // This includes both continental regions (North America, Europe) AND sub-regions (U.S., Canada, etc.)
          const isSingleRegionSelected = selectedGeos.length === 1 &&
                                         !selectedGeos.includes('Global') &&
                                         (mainRegions.includes(selectedGeos[0]) || subRegions.includes(selectedGeos[0]))
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 px-3 py-2 text-sm rounded-md transition-colors bg-blue-600 text-white cursor-default"
                disabled
              >
                Value
              </button>
